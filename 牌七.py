def com(card):
    ...
    for i in range(0,len(com_梅)):
        if int((com_梅[i])[1]) == int(梅[-1])+1 or int((com_梅[i])[1]) == int(梅[0])-1:
            goal.append("梅花"+((com_梅[i])[1]))
    for i in range(0,len(com_菱)):
        if int((com_菱[i])[1]) == int(菱[-1])+1 or int((com_菱[i])[1]) == int(菱[0])-1:
            goal.append("菱形"+((com_菱[i])[1]))
    for i in range(0,len(com_愛)) :
        if int((com_愛[i])[1]) == int(愛[-1])+1 or int((com_愛[i])[1]) == int(愛[0])-1:
            goal.append("愛心"+((com_愛[i])[1]))
    for i in range(0,len(com_黑)):
        if int((com_黑[i])[1]) == int(黑[-1])+1 or int((com_黑[i])[1]) == int(黑[0])-1:
    ...
    if card == "0":
        ...
        if len(player[int(kill)-1]) == 4:
           ...
           player_point.append(int(player_point[-1])+13)
            ...
            player_point.append(int(player[int(kill)-1][2]) + int(player_point[-1]))
...
while len(player) != 0 or len(p2) != 0 or len(p3) != 0 or len(p4) != 0:
    ...
    for i in range(0,len(player)):
        if i == len(梅花) or i == len(梅花)+len(菱形) or i == len(梅花)+len(菱形)+len(愛心) or i == len(梅花)+len(菱形)+len(愛心)+len(黑桃):
            print("\n")
        print(i+1,':',end="")
    ...
    if len(player) != 0:
        ...
        error = re.findall(r"[A-WY-Za-wy-z\u4e00-\u9fa5]+",ans)
        while len(error) > 0 or ans == "":
            ans = input('請輸入數字：')
            error = re.findall(r"[A-WY-Za-wy-z\u4e00-\u9fa5]+",ans)
        while count == 0:
            if ans == "0":
            ...
            if int(ans)-1 < len(梅花) and len(梅花)!=0:
                ...
                while int(a[1]) != int(梅[-1])+1 or int(a[1]) != int(梅[0])-1:
                    ...
                    if int(a[1]) != int(梅[-1])+1 or int(a[1]) != int(梅[0])-1:
                        ...
                        error = re.findall(r"[A-WY-Za-wy-z\u4e00-\u9fa5]+",ans)
                        while len(error) > 0 or ans == "" or int(ans) > len(player):
                            ans = input('請輸入數字：')
                            error = re.findall(r"[A-WY-Za-wy-z\u4e00-\u9fa5]+",ans)
                        change = player[int(ans)-1]
                        break
                ...
                if a[1] == "K":
                    a[1] = "13"
                while int(a[1]) != int(菱[-1])+1 or int(a[1]) != int(菱[0])-1:
                    ...
                    if int(a[1]) == int(菱[0])-1:
                        count+=1
                        break
                    if int(a[1]) != int(菱[-1])+1 or int(a[1]) != int(菱[0])-1:
                        ans = input("你現在還不能出這張牌,請輸入要出第幾張牌(蓋牌請輸入0):")
                        error = re.findall(r"[A-WY-Za-wy-z\u4e00-\u9fa5]+",ans)
                        while len(error) > 0 or ans == "" or int(ans) > len(player):
                            ans = input('請輸入數字：')
                ...
                if a[1] == "K":
                    a[1] = "13"
                while int(a[1]) != int(愛[-1])+1 or int(a[1]) != int(愛[0])-1:
                    print("eeeee")
                    ...
                    if int(a[1]) == int(愛[0])-1:
                        count+=1
                        break
                    if int(a[1]) != int(愛[-1])+1 or int(a[1]) != int(愛[0])-1:
                        ans = input("你現在還不能出這張牌,請輸入要出第幾張牌(蓋牌請輸入0):")
                        error = re.findall(r"[A-WY-Za-wy-z\u4e00-\u9fa5]+",ans)
                        while len(error) > 0 or ans == "" or int(ans) > len(player):
                            ans = input('請輸入數字：')
                            error = re.findall(r"[A-WY-Za-wy-z\u4e00-\u9fa5]+",ans)
                        break
            elif int(ans)-1 < len(梅花)+len(菱形)+len(愛心)+len(黑桃) and len(黑桃)!=0:
                change = player[int(ans)-1]
                a = change.split("黑桃")
                if a[1] == "A":
                    a[1] = "1"
                ...
                if a[1] == "K":
                    a[1] = "13"
                while int(a[1]) != int(黑[-1])+1 or int(a[1]) != int(黑[0])-1:
                    print("eeeee")
                    ...
                    if int(a[1]) == int(黑[0])-1:
                        count+=1
                        break
                    if int(a[1]) != int(黑[-1])+1 or int(a[1]) != int(黑[0])-1:
                        ans = input("你現在還不能出這張牌,請輸入要出第幾張牌(蓋牌請輸入0):")
                        error = re.findall(r"[A-WY-Za-wy-z\u4e00-\u9fa5]+",ans)
                        while len(error) > 0 or ans == "" or int(ans) > len(player):
                            ans = input('請輸入數字：')
...
total = [player_point,p2_point,p3_point,p4_point]
tt = [player_point,p2_point,p3_point,p4_point]
winner = ["player1","player2","player3","player4"]
total.sort()
for i in range(0,4):
    if total[0] == tt[i]:
        print("第一名","\t",winner[i],"共蓋了:",(tt[i])[-1],"點","蓋的牌:",to[i])
    if total[1] == tt[i]:
        print("第二名","\t",winner[i],"共蓋了:",(tt[i])[-1],"點","蓋的牌:",to[i])
    if total[2] == tt[i]:
        print("第三名","\t",winner[i],"共蓋了:",(tt[i])[-1],"點","蓋的牌:",to[i])
    if total[3] == tt[i]:
        print("第四名","\t",winner[i],"共蓋了:",(tt[i])[-1],"點","蓋的牌:",to[i])
